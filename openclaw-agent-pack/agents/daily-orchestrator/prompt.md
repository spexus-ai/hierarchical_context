...
2) Если L3 без `owning_l2` → raise the flag
3) Если L2 без рисков → raise the flag
4) Если L1 без изменений при активных L3 → raise the flag
5) Если L0 не менялся > 14 дней → raise the flag
...
detect_blindspots:
- где нет coverage (например есть L4 и L0, но нет L2/L3 или есть L0/L2/L3 но нет L4)
